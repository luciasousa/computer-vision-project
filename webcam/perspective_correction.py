...
for i in range(len(corners)):
    ...
    #print(ids)
    #draw circle in left up corner of the marker with the id number equal to 1
    if ids[i] == 1:
        cv2.circle(frame, tuple(c[3].astype('int32')), 5, (255,0,0), -1)
        x1,y1 = c[3].astype('int32')
        print("x1,y1= " + str(x1)+ " " +str(y1))
    #draw circle in right up corner of the marker with the id number equal to 2
    if ids[i] == 2:
        cv2.circle(frame, tuple(c[2].astype('int32')), 5, (255,0,0), -1)
        x2,y2 = c[2].astype('int32')
        print("x2,y2= " + str(x2)+ " " +str(y2))
    #draw circle in left down corner of the marker with the id number equal to 3
    if ids[i] == 3:
        cv2.circle(frame, tuple(c[0].astype('int32')), 5, (255,0,0), -1)
        x3,y3 = c[0].astype('int32')
        print("x3,y3= " + str(x3)+ " " +str(y3))
    #draw circle in right down corner of the marker with the id number equal to 4
    if ids[i] == 4:
...
dst = cv2.warpPerspective(frame,M,(502,500))

gray_scale=cv2.cvtColor(dst,cv2.COLOR_BGR2GRAY)
th1,img_bin = cv2.threshold(gray_scale,150,225,cv2.THRESH_BINARY)
#cv2.imshow("Image binary dst", dst)
...
#cv2.imshow("Image dilate", ~img_bin_final)
ret, labels, stats, centroids = cv2.connectedComponentsWithStats(~img_bin_final, connectivity=8, ltype=cv2.CV_32S)
#print(stats[2:])
### skipping first two stats as background
for x,y,w,h,area in stats[2:]:
    # x cresce para a direita e o y para baixo
    #if x>=x1 and x<=x2 and y>=y1 and y<=y3: #ver estas coordenadas, estao mal
        # estao bem para as imagens sem a perpetive correction mas com ela, nao funcionam, pq sao as coordenadas da imagem original
