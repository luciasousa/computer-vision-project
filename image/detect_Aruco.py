...
for i in range(len(corners)):
    ...
    cv2.putText(frame, str(c[0]), tuple(c[0].astype('int32')), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
    cv2.putText(frame, str(c[1]), tuple(c[1].astype('int32')), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
    cv2.putText(frame, str(c[2]), tuple(c[2].astype('int32')), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
    cv2.putText(frame, str(c[3]), tuple(c[3].astype('int32')), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)

    #draw circle in left down corner of the marker with the id number equal to 1
    if ids[i] == 1:
        cv2.circle(frame, tuple(c[3].astype('int32')), 5, (0,0,255), -1)
        x1,y1 = c[3].astype('int32')
    #draw circle in right down corner of the marker with the id number equal to 2
    if ids[i] == 2:
        cv2.circle(frame, tuple(c[2].astype('int32')), 5, (0,0,255), -1)
        x2,y2 = c[2].astype('int32')
    #draw circle in left up corner of the marker with the id number equal to 3
    if ids[i] == 3:
        cv2.circle(frame, tuple(c[0].astype('int32')), 5, (0,0,255), -1)
        x3,y3 = c[0].astype('int32')
    #draw circle in right up corner of the marker with the id number equal to 4
    if ids[i] == 4:
        cv2.circle(frame, tuple(c[1].astype('int32')), 5, (0,0,255), -1)
